...
switch ($query) {
  ...
  case "nolabel":
    $sql="select items.id as ID,typedesc as type, agents.title as manufacturer ,sn as serial,model , statusdesc as status ,username as user".
         " FROM items,itemtypes,agents,statustypes, users ".
         " WHERE agents.id=manufacturerid AND itemtypes.id=items.itemtypeid AND statustypes.id=status AND users.id=items.userid AND (label='' OR label is null)";
    $editlnk="$scriptname?action=edititem&id";
  break;
