<?php
	 ...
	 */
	$aColumns = array('itemid','typedesc','title','itemmodel','serial','itemlabel','username',
	'division','locationname','areaname','problem','solution','replacement','repairdate','finishdate','statusrepair');
	...
	if ( $sWhere != "" )
	{
	      ...
	      $sQueryCnt = "
		  SELECT count(itemsrepair.id) as count ,
		  itemsrepair.id AS itemid,
		  itemsrepair.model AS itemmodel,
		  itemsrepair.label AS itemlabel,
		  itemsrepair.division AS division,
		  itemsrepair.problem AS problem,
		  itemsrepair.solution AS solution,
		  itemsrepair.replacement AS replacement,
		  itemsrepair.repairdate AS repairdate,
		  itemsrepair.finishdate AS finishdate,
		  itemsrepair.statusrepair AS statusrepair,
                  ...
                  (SELECT group_concat( tags.name ,',') from tags,tag2item WHERE tag2item.itemid=itemsrepair.id AND tags.id=tag2item.tagid) AS taginfo,
                  (SELECT group_concat( software.stitle ,'|') from software,item2soft WHERE item2soft.itemid=itemsrepair.id AND software.id=item2soft.softid) AS softinfo
                  FROM
                  itemsrepair
		  JOIN itemtypes ON itemsrepair.itemtypeid=itemtypes.id 
		  JOIN agents ON itemsrepair.manufacturerid=agents.id
		  JOIN users ON itemsrepair.userid=users.id
		  JOIN statustypes ON itemsrepair.status=statustypes.id
		  LEFT OUTER JOIN locations ON itemsrepair.locationid=locations.id
		  LEFT OUTER JOIN locareas ON itemsrepair.locareaid=locareas.id
		  LEFT OUTER JOIN racks ON itemsrepair.rackid=racks.id
		  $sWhere";
	      $sth=db_execute($dbh,$sQueryCnt);
	      $rResultTotal=$sth->fetch(PDO::FETCH_ASSOC);
	...
	$sQuery = "
		  ...
		  JOIN itemtypes ON itemsrepair.itemtypeid=itemtypes.id 
		  JOIN agents ON itemsrepair.manufacturerid=agents.id
		  LEFT OUTER JOIN statustypes ON itemsrepair.status=statustypes.id
		  JOIN users ON itemsrepair.userid=users.id
		  LEFT OUTER JOIN locations ON itemsrepair.locationid=locations.id
		  LEFT OUTER JOIN locareas ON itemsrepair.locareaid=locareas.id
		  LEFT OUTER JOIN racks ON itemsrepair.rackid=racks.id
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			if ( $aColumns[$i] == "itemid" ) {
				...
				$r="<div style='width:60px'><span $attr>&nbsp;</span>".
				   "<span><a class='editid' title='Edit' href='?action=editrepair&amp;id=".$aRow['itemid']."'>".
